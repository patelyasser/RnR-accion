...
/**
* Hide the aside to be toggled via click.
*/
@Directive({
  selector: '[appHideAsideMenu]',
  providers: [ClassToggler]
})
export class HideAsideDirective implements OnInit {
  @Input('appHideAsideMenu') breakpoint: string;
  public bp;
  constructor(private classToggler: ClassToggler) {}
  ngOnInit(): void {
    this.bp = this.breakpoint;
  ...
  @HostListener('click', ['$event'])
  toggleOpen($event: any) {
    this.classToggler.removeClass('aside-menu-lg-show');
  }
}
